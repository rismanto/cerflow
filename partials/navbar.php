...
<nav class="bg-white border-b-2 border-slate-300 px-8 py-4 flex justify-between items-center sticky top-0 z-50 shadow-sm">
    <div class="flex items-center gap-4">
        ...
        <div id="active-title-container" class="hidden flex items-center gap-2">
            ...
            <div id="active-title" class="text-blue-700 font-black text-sm uppercase italic tracking-tight bg-blue-50 px-4 py-2 border border-blue-200 shadow-inner">
                <?php echo isset($activeTitle) ? $activeTitle : ''; ?>
            </div>
            <button onclick="backToSelector()" class="bg-white border-2 border-slate-300 text-slate-600 hover:text-blue-700 hover:border-blue-400 px-4 py-2 text-xs font-black uppercase tracking-widest transition-all ml-2">
                Ganti Materi
            </button>
        </div>
        <?php endif; ?>
    ...
    <div class="flex items-center gap-3">
        ...
        <?php elseif ($role == 'siswa'): ?>
            <?php if ($navContext === 'history' || $navContext === 'report'): ?>
                <a href="siswa.php" class="bg-slate-800 text-white px-6 py-2 text-sm font-bold shadow hover:bg-black transition-all mr-2">← Kembali ke Workspace</a>
            <?php else: ?>
                <a href="history.php" class="text-sm font-bold text-slate-600 hover:text-blue-700 hover:bg-blue-50 px-4 py-2 border border-transparent hover:border-blue-200 transition-all mr-2">Hasil Sebelumnya</a>
            <?php endif; ?>
            <div id="editor-actions" class="hidden flex gap-3">
                <?php if (!isset($allowFeedbackDuringWork) || $allowFeedbackDuringWork == '1'): ?>
                    <button id="btn-feedback" onclick="toggleFeedback()" class="bg-white border-2 border-slate-300 text-slate-700 px-6 py-2 text-sm font-bold hover:bg-slate-50 hover:border-slate-400 transition-all">💡 FEEDBACK</button>
                <?php endif; ?>
        ...
        <?php if ($role): ?>
            <a href="logout.php" class="text-sm font-bold text-red-600 hover:bg-red-50 px-4 py-2 border-2 border-red-200 hover:border-red-400 transition-all ml-2">LOGOUT</a>
        <?php endif; ?>
    </div>
</nav>
